...
class FileInfoAndSelectorBox(QWidget):
	def __init__(self, type, extension = "", maxSelect = 100):
		...
		self.labelSelect.setAlignment(Qt.AlignmentFlag.AlignCenter)
		...
		self.twFlies.setAlignment(Qt.AlignmentFlag.AlignCenter)
	...
	def on_click_select(self):
		...
		fileName, _ = QFileDialog.getOpenFileNames(self,"Select input files", open_loc,f"All Files (*);;{self.fileType} Files (*.{self.ext})")
		if fileName:
			print(fileName)
